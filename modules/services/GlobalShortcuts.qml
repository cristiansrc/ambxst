...
QtObject {
    ...
    function toggleSettings(screenName) {
        console.log("[GlobalShortcuts] toggleSettings called with screenName:", screenName);
        const willOpen = !GlobalStates.settingsWindowVisible;
        if (willOpen) {
            let targetScreen = screenName || AxctlService.focusedMonitor?.name || "";
            let targetWorkspaceId = 0;
            
            if (targetScreen) {
                const monitor = AxctlService.monitorFor(targetScreen);
                console.log("[GlobalShortcuts] monitor found for targetScreen:", targetScreen, "is:", monitor ? monitor.name : "null");
                if (monitor && monitor.activeWorkspace) {
                    targetWorkspaceId = monitor.activeWorkspace.id;
                }
            }
            
            if (targetWorkspaceId === 0) {
                targetWorkspaceId = AxctlService.focusedMonitor?.activeWorkspace?.id || AxctlService.focusedWorkspace?.id || 0;
            }

            console.log("[GlobalShortcuts] Target Workspace ID:", targetWorkspaceId, "Target Screen Name:", targetScreen);
            GlobalStates.settingsTargetWorkspaceId = targetWorkspaceId;
            GlobalStates.settingsTargetScreenName = targetScreen;
