...
Singleton {
    ...
    property GeminiWebStrategy geminiWebStrategy: GeminiWebStrategy {}
    ...
    function getStrategyForProvider(providerName) {
        ...
        case "gemini-web": return geminiWebStrategy;
...
`;
    ...
    function fetchAvailableModels() {
        ...
        // Gemini Web (servidor local gemini-web-to-api)
        // Intentar conectar al servidor local, si está disponible
        pendingFetches++;
        fetchProcessGeminiWeb.command = ["bash", "-c", "curl -s --connect-timeout 2 http://localhost:4981/gemini/v1beta/models || echo '{\"models\":[]}'"];
        fetchProcessGeminiWeb.running = true;
    ...
    // Gemini Web (servidor local gemini-web-to-api)
    Process {
        id: fetchProcessGeminiWeb
        stdout: StdioCollector {
            id: fetchGeminiWebOut
        }
        onExited: exitCode => {
            if (exitCode === 0) {
                try {
                    let data = JSON.parse(fetchGeminiWebOut.text);
                    if (data.models) {
                        let newModels = [];
                        for (let i = 0; i < data.models.length; i++) {
                            let item = data.models[i];
                            let id = item.name ? item.name.replace("models/", "") : (item.id || "gemini-advanced");
                            // Filtrar solo modelos Gemini
                            if (id.includes("gemini") || id.includes("flash") || id.includes("pro") || id === "gemini-advanced") {
                                let m = aiModelFactory.createObject(root, {
                                    name: item.displayName || (item.name || "Gemini Advanced (Web)"),
                                    icon: Qt.resolvedUrl("../../../assets/aiproviders/google.svg"),
                                    description: item.description || "Google Gemini via Web (cookies)",
                                    endpoint: "http://localhost:4981/gemini/v1beta",
                                    model: id,
                                    provider: "gemini-web",
                                    requires_key: false
                                });
                    ...
                    console.log("Gemini Web fetch error: " + e);
                }
            }
            checkFetchCompletion();
        }
