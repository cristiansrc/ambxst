...
Singleton {
    ...
    Process {
        ...
        command: ["bash", "-c", "bluetoothctl show"]
        running: false
        property string buffer: ""
        stdout: SplitParser {
            onRead: (data) => {
                checkPowerProcess.buffer += data + "\n";
            }
        }
        onExited: (exitCode) => {
            const output = checkPowerProcess.buffer.trim();
            checkPowerProcess.buffer = "";
            
            root.available = output !== "" && !output.includes("No default controller available");
