...
PanelWindow {
    ...
    signal lockscreenFrameReady(string filePath)
    ...
    function updateMpvRuntime(enable) {
        var cmdJson = {
            "command": ["set_property", "glsl-shaders", enable ? mpvShaderPath : ""]
        };
        var payload = JSON.stringify(cmdJson) + "\n";
        var pythonCode = "import socket, sys; " +
                         ...
                         "finally: " +
                         "  s.close()";
        mpvIpcProcess.command = ["python3", "-c", pythonCode, mpvSocket, payload];
    ...
    Process {
        id: mpvSyncProcess
        running: false
        command: ["python3", "-c", "import socket, glob; " +
                                   "payload = '{\\\"command\\\": [\\\"set_property\\\", \\\"time-pos\\\", 0]}\\n'.encode('utf-8'); " +
                                   "for sock_path in glob.glob('/tmp/ambxst_mpv_socket_*'): " +
                                   "    try: " +
                                   "        s = socket.socket(socket.AF_UNIX, socket.SOCK_STREAM); " +
                                   "        s.connect(sock_path); " +
                                   "        s.sendall(payload); " +
    ...
    function getRandomVideo() {
        var videos = wallpaperPaths.filter(function(path) {
            return getFileType(path) === 'video';
        });
    ...
    function getRandomWallpaper() {
        ...
        var randomIndex = Math.floor(Math.random() * wallpaperPaths.length);
        return wallpaperPaths[randomIndex];
    }
    ...
    function selectNextWallpaper() {
        ...
        if (Quickshell.screens.length > 1) {
            ...
            var candidates = [];
            if (wallpaperAdapter.randomVideoOnly) {
                candidates = wallpaperPaths.filter(function(p) {
                    return getFileType(p) === 'video';
                });
            } else {
                candidates = wallpaperPaths;
            }

            if (candidates.length === 0) {
                console.warn("No wallpaper candidates found for multi-screen change.");
                return;
            }

            // Shuffle candidates list to get different distinct items per monitor
            ...
            for (var k = 0; k < Quickshell.screens.length; k++) {
                var screenName = Quickshell.screens[k].name;
                var path = "";
                
                if (wallpaperAdapter.randomMode || wallpaperAdapter.randomVideoOnly) {
                    // Grab from shuffled list, looping if we have fewer candidates than screens
                    path = shuffled[k % shuffled.length];
                } else {
                    // Sequential: next index offset
                    var seqIndex = (currentIndex + k) % wallpaperPaths.length;
                    path = wallpaperPaths[seqIndex];
                }
                
                if (path !== "") {
                    perScreen[screenName] = path;
                }
            }

            wallpaperConfig.adapter.perScreenWallpapers = perScreen;

            // Sync the main properties (currentIndex/currentWallpaper) to the primary monitor
            if (GlobalStates.primaryScreenName !== "") {
                var firstScreenName = GlobalStates.primaryScreenName;
                var firstPath = perScreen[firstScreenName];
                if (firstPath) {
                    var pathIndex = wallpaperPaths.indexOf(firstPath);
                    if (pathIndex !== -1) {
                        currentIndex = pathIndex;
                        currentWallpaper = firstPath;
                        wallpaperConfig.adapter.currentWall = firstPath;
                    }
                }
            }
            ...
            // Single screen
            if (wallpaperAdapter.randomVideoOnly) {
                var vid = getRandomVideo();
                if (vid !== "") {
                    setWallpaper(vid);
                }
            } else if (wallpaperAdapter.randomMode) {
    ...
    Timer {
        ...
        running: GlobalStates.wallpaperManager === wallpaper && wallpaperAdapter.autoChangeEnabled && wallpaperPaths.length > 0
        repeat: true
        onTriggered: {
            console.log("Auto-changing wallpaper via timer");
            selectNextWallpaper();
        }
    }
    ...
    FileView {
        ...
        JsonAdapter {
            ...
            property int changeInterval: 900
            property bool randomMode: false
            property bool randomVideoOnly: false
            property bool staticWallMode: false
    ...
    Process {
        ...
        onExited: function (exitCode) {
            if (exitCode === 0) {
                ...
                wallpaper.lockscreenFrameReady(wallpaper.baseWallpaper);
            } else {
