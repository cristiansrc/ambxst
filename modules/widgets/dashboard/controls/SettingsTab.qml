...
Rectangle {
    ...
    property int currentSection: NetworkService.wifiAvailable ? 0 : (BluetoothService.available ? 1 : 2)
    ...
    // Filtered sections based on search query and hardware availability
    readonly property var filteredSections: {
        let baseModel = sectionModel.filter(s => {
            if (s.section === 0 && !NetworkService.wifiAvailable) return false;
            if (s.section === 1 && !BluetoothService.available) return false;
            return true;
        });
        ...
        return searchIndex.items.filter(item => {
            if (item.section === 0 && !NetworkService.wifiAvailable) return false;
            if (item.section === 1 && !BluetoothService.available) return false;
            return fuzzyMatch(query, item.label) || (item.keywords && item.keywords.includes(query));
        }).map(item => {
            // Find section metadata
            const sectionMeta = baseModel.find(s => s.section === item.section) || {};
            return {
                label: item.label,
