...
ApiStrategy {
    ...
    // Conecta al servidor local gemini-web-to-api
    // Endpoint compatible con Gemini API nativo
    function getEndpoint(modelObj, apiKey) {
        // El servidor local expone endpoints compatibles con Gemini
        // http://localhost:4981/gemini/v1beta/models/{model}:generateContent
        let baseUrl = "http://localhost:4981/gemini/v1beta/models";
        let modelId = modelObj.model || "gemini-advanced";
        return baseUrl + "/" + modelId + ":generateContent";
    }

    function _getStreamEndpoint(modelObj, apiKey) {
        let baseUrl = "http://localhost:4981/gemini/v1beta/models";
        let modelId = modelObj.model || "gemini-advanced";
        return baseUrl + "/" + modelId + ":streamGenerateContent?alt=sse";
    }

    function getHeaders(apiKey) {
        // El servidor local no requiere autenticación
        // Solo necesita Content-Type
    ...
    function getStreamBody(messages, model, tools) {
        // Gemini streaming usa el mismo body, solo cambia el endpoint
